...
def ensure_laplacians(net: Any, normalize: bool = False) -> None:
    """
    Ensure the temporal network exposes Laplacian matrices for the requested
    normalization mode.
    """

    requested_random_walk = bool(normalize)
    cached_random_walk = getattr(net, "_laplacians_random_walk", None)
    ...
    net._laplacians_random_walk = requested_random_walk


def _iter_laplacians(net: Any) -> list[Any]:
    laplacians = getattr(net, "laplacians", None)
...
def _smallest_eigen_signature(L: Any, n_eigen: int) -> np.ndarray:
    """
    Return a normalized signature made of the `n_eigen` smallest eigenvalues.
    """
...
def compute_laplacian_signatures(
    ...
    """
    Compute the normalized Laplacian signatures for every snapshot.
    """

    ensure_laplacians(data, normalize=normalize)
    laplacians = _iter_laplacians(data)

    if not laplacians:
        return np.empty((0, int(n_eigen)), dtype=float)

    signatures = np.empty((len(laplacians), int(n_eigen)), dtype=float)
...
def compute_similarity_signal_from_signatures(
    ...
    """
    Compute the Laplacian similarity signal once the per-snapshot signatures
    ...
    num_snapshots = laplacian_signatures.shape[0]
