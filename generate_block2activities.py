...
PHASE_1_EVENT_SEED_BASE = 1415
PHASE_2_EVENT_SEED_BASE = 8281
SBM_SEED_BASE = 271

TRAIN_OUTPUT_PATH = Path("data/block2activities_train.pkl")
TEST_OUTPUT_PATH = Path("data/block2activities_test.pkl")


def generate_piecewise_activity(sample_index: int, t_split: int) -> tuple[int, np.ndarray, np.ndarray]:
    n_phase_1, starts_phase_1, ends_phase_1 = EDLDE.EDLDE(
        density=density_1,
        inter_tau=inter_tau_1,
        t_start=t_start,
        t_end=t_split,
        seed=PHASE_1_EVENT_SEED_BASE + sample_index,
    )
    n_phase_2, starts_phase_2, ends_phase_2 = EDLDE.EDLDE(
        ...
        t_start=t_split,
        t_end=t_end,
        seed=PHASE_2_EVENT_SEED_BASE + sample_index,
    )
...
def generate_sample(sample_index: int, t_split: int) -> dict[str, object]:
    number_of_events, starting_times, ending_times = generate_piecewise_activity(
