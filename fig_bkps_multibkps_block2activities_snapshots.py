...
def get_signal_path(signals_outdir: Path, sample_name: str, lamda: float, window: float) -> Path:
    return (
        signals_outdir
        / sample_name
        / f'signal_lamda_{float(lamda):.11f}_window_{float(window):g}.pkl'
    )
...
for sample in range(n_samples):
    ...
    x_values = np.asarray(signal.get('k_samples', np.arange(len(signal_values))))
